...
* After playing with [function interposing]({% capture itemLink %}{% include findCollectionItem.html collectionName='tips' itemTitle='Function Interposing' %}{% endcapture %}{{ itemLink | strip_newlines }}) to backdoor the random number generator, I wanted to apply the same  *__load-time code injection__* technique to a proper Objective-C application.
