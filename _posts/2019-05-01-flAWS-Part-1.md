...
Tot avoid the <i><b>snapshotId can only be modified on EBS devices</b></i> error when attaching the volume, search by snapshot id (<i>snap-0b49342abd1bdcb89</i>) and use an available AWS recommended device name like <code>/dev/sdf</code>.  AWS recommended device names for attaching EBS volumes are <p>/dev/sd[f-p]</p>
</div>
