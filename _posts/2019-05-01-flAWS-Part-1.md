...
<div class="box-warning">
  Tot avoid the <i>snapshotId can only be modified on EBS devices</i> error when attaching the volume, search by snapshot id (<i>snap-0b49342abd1bdcb89</i>) and use an available AWS recommended device name like <code>/dev/sdf</code>.  AWS recommended device names for attaching EBS volumes are <code>/dev/sd[f-p]</code>
</div>
